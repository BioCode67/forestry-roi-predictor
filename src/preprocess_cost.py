...
#   ROI = 소득 / 경영비,  소득 = 총수입(총생산액) - 경영비
#
# 산출(revenue-side)은 예외 없이 배제한다. 금액뿐 아니라 **수량**도 마찬가지다.
# 수량 × 단가 = 총수입이므로 수량을 남기면 목표변수가 거의 그대로 복원된다.
# 실제로 이 규칙을 느슨하게 두었을 때 총생산액·부가가치·등급별 수량이 섞여 들어가
# Test R²가 0.91까지 부풀려졌다. 아래는 그 재발을 막기 위한 규칙이다.
LEAKY_PATTERNS = [
...
# 시비량(무기질/유기질 수량)은 산출이 아니라 **투입량**이므로 유지한다.
