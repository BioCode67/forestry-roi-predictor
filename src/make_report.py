"""
Phase 3 — 공모전 [붙임 3] 데이터 분석 부문 결과보고서 자동 생성

모든 수치는 models/*.json, data/processed_*_meta.json 에서 주입한다.
재학습 후 `python src/make_report.py` 한 번이면 보고서가 갱신된다.
...
# ---------------------------------------------------------------------------
def load(name: str, base: str = MODEL_DIR, required: bool = True):
    p = os.path.join(base, name)
    if not os.path.exists(p):
        if required:
            raise FileNotFoundError(f"{p} 가 없습니다. 해당 스크립트를 먼저 실행하세요.")
        return None
...
def bench_table(m: dict) -> str:
    labels = [
        ("forest_service_baseline", "① 산림청 단순평균 (현행 공표 방식)"),
        ("linear_regression", "② 다중 선형회귀"),
        ("optuna_xgboost", "③ **Optuna-XGBoost (제안 모델)**"),
    ]
    return "\n".join(
        f"| {lab} | {f(m[k]['test']['R2'])} | {f(m[k]['test']['RMSE'], 2)} | "
        f"{f(m[k]['test']['MAE'], 2)} |"
        for k, lab in labels if k in m
    )


# ---------------------------------------------------------------------------

def renumber(doc: str) -> str:
    """소제목 번호를 다시 매깁니다.

    각 블록이 자기 번호를 문자열에 박고 있어, 블록을 새로 끼우면 4-4가 두 번
    나오는 식으로 어긋납니다. 번호를 손으로 맞추면 다음에 또 어긋납니다.
    쓰인 순서대로 다시 매기는 편이 낫습니다.
    """
    import re
    ...
    def fix(m):
        ...
        return f"### {sec}-{counters[sec]}. {m.group(3)}"

    return re.sub(r"### (\d)-(\d+)\. (.+)", fix, doc)


def main() -> None:
    ma = load("metrics_summary.json")
    mb = load("metrics_cost.json", required=False)
    mq = load("metrics_quantile.json", required=False)
    ins = load("insights.json", required=False)
    prod = load("production_insights.json", required=False)
    mgmt = load("management_insights.json", required=False)
    subs = load("subsidy_programs.json", required=False)
    meta_a = load("processed_meta.json", DATA_DIR)
    meta_b = load("processed_cost_meta.json", DATA_DIR, required=False)

    dsa, oxa = ma["dataset"], ma["optuna_xgboost"]
    impa = ma["improvement_vs_baseline"]

    # ── 4-2 변수 중요도 -----------------------------------------------------
    gi_a = "\n".join(
        f"| {i} | {k} | {f(v, 0)} |"
        for i, (k, v) in enumerate(list(ma.get("feature_importance_gain", {}).items())[:10], 1))

    # ── Model B -------------------------------------------------------------
    b_block = ""
    if mb:
        dsb, oxb, impb = mb["dataset"], mb["optuna_xgboost"], mb["improvement_vs_baseline"]
        per_item = "\n".join(
            f"| {it} | {v['n']:,} | {f(v['R2'], 3)} | {f(v['RMSE'], 1)} | {f(v['MAE'], 1)} |"
            for it, v in sorted(mb.get("per_item_test", {}).items(), key=lambda kv: -kv[1]["R2"]))
        gi_b = "\n".join(
            f"| {i} | {k} | {f(v, 0)} |"
            for i, (k, v) in enumerate(list(mb.get("feature_importance_gain", {}).items())[:12], 1))
        items_line = " · ".join(f"{k} {v:,}" for k, v in dsb["items"].items())
...
| 분석 표본 | **{dsb['rows']:,}행** ({items_line}) |
| 목표변수 | {dsb['target_definition']} |
| 설명변수 | {dsb['n_features']}개 — 비목별 지출, 작업 공정별 노동시간, 수령·재배면적·재배본수 |
| 단위 | {dsb['unit_note']} |

#### 성능 (Test set, n = {dsb['test']:,})
...
베이스라인 대비 **RMSE {impb['RMSE_reduction_pct']:.1f}%, MAE {impb['MAE_reduction_pct']:.1f}% 감소**.
교차검증 OOF R² {f(oxb['cv_oof']['R2'])} 가 Test {f(oxb['test']['R2'])} 와 유사해 과적합이 없다.
...
| 관측 단위 | 임가 전체 | 품목 × 단위면적 |
| 설명변수 | {dsa['n_features']}개 (구조 변수 중심) | {dsb['n_features']}개 (비목·공정 포함) |
| Test R² | **{f(oxa['test']['R2'])}** | **{f(oxb['test']['R2'])}** |

같은 임업 수익성인데 설명력이 {oxb['test']['R2'] / max(oxa['test']['R2'], 1e-9):.1f}배 차이난다.
...
"""
    ...
    if mq:
        rows = []
        for key, label in [("roi", "Model A (임가경제조사)"), ("cost", "Model B (임산물생산비조사)")]:
            q = mq.get(key)
            if q:
                rows.append(
                    f"| {label} | {q['n_test']:,} | {pct(q['coverage_80pct'] * 100)} | "
                    f"{f(q['median_interval_width'], 1)}%p | {f(q['p50_mae'], 2)}%p |")
        q_block = f"""
...
"""
    ...
    ins_block = ""
    if ins:
        g = ins.get("등급별_단가", {}).get("밤")
        sim = ins.get("등급전환_시뮬레이션", {}).get("밤")
        ages = ins.get("수령별_수익성", {})
        gaps = ins.get("선도임가_격차", {})
        grade_rows = "\n".join(
            f"| {r['구분']} | {f(r['단가_원per단위수량'], 0)} | {f(r['생산임가_비율_pct'], 0)}% |"
            for r in (g["등급"] if g else []))
        age_rows = "\n".join(
            f"| {k} | {v['최고구간']} ({f(v['최고ROI'], 0)}%) | {v['최저구간']} ({f(v['최저ROI'], 0)}%) |"
...
최고/최저 **{f(g['최고_최저_단가배수'], 2) if g else '—'}배** 격차다.
{f"선별 강화로 {sim['전환_시나리오']} 시 단위면적당 수취액이 **{sim['수취액_증가_원per단위면적']:+,.0f}원** 늘어난다." if sim else ""}
...
"""
    ...
    if prod:
        ...
        spec = prod["지역특화도_LQ"]
        trend_rows = "\n".join(
            f"| {i} | {f(tr[i]['연도별'][0]['가중평균단가'], 0)} | "
            f"{f(tr[i]['연도별'][-1]['가중평균단가'], 0)} | "
            f"{tr[i]['단가_변화율_pct']:+.1f}% | {tr[i]['생산량_변화율_pct']:+.1f}% |"
            for i in FOCUS if i in tr)
        prem_rows = "\n".join(
            f"| {i} | {prem[i]['최고지역']} {f(prem[i]['최고단가'], 0)} | "
            f"{prem[i]['최저지역']} {f(prem[i]['최저단가'], 0)} | {f(prem[i]['지역격차_배'], 2)}배 |"
            for i in FOCUS if i in prem)
        lq_rows = "\n".join(
            f"| {i} | {spec[i]['상위지역'][0]['시도']} | {f(spec[i]['상위지역'][0]['LQ'], 2)} | "
            f"{f(spec[i]['상위지역'][0]['전국비중_pct'], 1)}% |"
            for i in FOCUS if i in spec and spec[i]["상위지역"])
        proc = prod.get("가공부가가치", {})
        proc_txt = ""
        for k, v in proc.items():
            proc_txt += (
                f"\n**{k} — {v['판정']}**\n\n{v['해석']} 원물 1kg을 가공해 얻는 수취액은 "
                f"{f(v['원물1kg당_가공수취액'], 0)}원으로 원물 직판({f(v['원물_단가'], 0)}원) 대비 "
                f"**{v['원물직판대비_pct']:+.1f}%**다. {v['주의']}\n")
        prod_block = f"""
### 4-9. 임산물 시장 구조 — 전 품목 지역별 실측 단가

KAMIS 일일 가격조사는 63개 품목만 다루며 임산물은 단감·느타리·새송이·팽이버섯뿐이다.
밤·대추·떫은감·표고·산나물의 가격 정보가 통째로 비어 있었는데,
「임산물생산조사」 전품목({prod['연도'][0]}~{prod['연도'][-1]}년, {prod['관측']:,}개 시군구-품목 관측)이
이 공백을 임업통계만으로 메운다.

#### 단가·생산량 추이

| 품목 | {prod['연도'][0]}년 (원/kg) | {prod['연도'][-1]}년 | 단가 변화 | 생산량 변화 |
|---|---:|---:|---:|---:|
{trend_rows}
...
동일 품목인데도 산지에 따라 **최대 {max((prem[i]['지역격차_배'] for i in FOCUS if i in prem), default=0):.1f}배**
...
"""
    ...
    if mgmt:
        rows = []
        for crop in ["밤", "떫은감"]:
            e = mgmt["품목별"].get(crop, {}).get("출하시기별_단가")
            if e:
                rows.append(
                    f"| {crop} | {e['최고시기']} {f(e['최고단가'], 0)}원 | "
                    f"{e['최저시기']} {f(e['최저단가'], 0)}원 | {f(e['격차_배'], 2)}배 |")
        ch_rows = []
        for crop in ["밤", "떫은감", "버섯"]:
            e = mgmt["품목별"].get(crop, {}).get("판매처별_단가")
            if e:
                ch_rows.append(
                    f"| {crop} | {e['최고판매처']} {f(e['계열단가'][e['최고판매처']], 0)}원 | "
                    f"{e['최저판매처']} {f(e['계열단가'][e['최저판매처']], 0)}원 | {f(e['격차_배'], 2)}배 |")
        st_rows = []
        for crop in ["밤", "떫은감"]:
            c = mgmt["품목별"].get(crop, {})
            s, ce = c.get("저장경험별_단가"), c.get("공식인증_프리미엄")
            if s:
                st_rows.append(
                    f"| {crop} | {s['단가차_pct']:+.1f}% | "
                    f"{ce['프리미엄_pct']:+.1f}% |" if ce else
                    f"| {crop} | {s['단가차_pct']:+.1f}% | — |")
        mgmt_block = f"""
...
"""
    ...
    if subs:
        cat_rows = "\n".join(
            f"| {c} | {v['사업수']} | {v['자부담_최저_pct']}% | "
            f"{f(v['최대_지렛대배수'], 1)}배 | {v['대표사업_최저자부담']} |"
            for c, v in sorted(subs["분류별_요약"].items(), key=lambda kv: kv[1]["자부담_최저_pct"]))
        subs_block = f"""
...
산림청 「보조금 세부사업 정보」 {subs['사업수']}개 사업(폐지 {subs['제외_폐지사업']}건 제외)을
...
**다만 지렛대는 위험도 함께 키운다.** {subs['유의']}
대시보드는 실효 ROI와 함께 이 경고를 항상 표시한다.
"""

    # ── 패널 구조 · 분할 감사 · 작목 조합 ────────────────────────────────
    mp = load("metrics_panel.json", required=False)
    panel_block = ""
    if mp:
        g = mp["임가단위_5회평균"]
        sd = mp["임가단위_fold표준편차"]
        prows = "\n".join(
            f"| {k} | {f(g[k]['R2'])} | {f(g[k]['RMSE'], 2)} | {f(g[k]['MAE'], 2)} | ±{f(sd[k])} |"
            for k in ("산림청 방식", "기존 22변수", "작년값 선형보정", "패널 변수 추가")
            if k in g)
        yr = mp.get("연도단위", {})
        yrows = "\n".join(
            f"| ~{int(t)-1}년 학습 → {t}년 시험 | {v['학습']:,} | {v['시험']:,} | "
            f"{f(v['산림청 방식']['R2'])} | {f(v['기존 22변수']['R2'])} | {f(v['패널 변수 추가']['R2'])} |"
            for t, v in sorted(yr.items()))
        ap = mp["적용범위"]
        pb = g.get("패널 변수 추가", {})
        ratio_p = pb.get("R2", 0) / g["산림청 방식"]["R2"] if g["산림청 방식"]["R2"] else 0
        panel_block = f"""
### 4-4. 패널 구조 — 작년 실적이 가진 힘

임가경제조사는 같은 임가를 여러 해 따라가는 패널이다(임가 {ap['임가수']:,}곳 / 행 {ap['전체_행']:,}).
그런데 지금까지의 모델은 각 행을 독립 관측으로만 다뤘다. **작년에 이 임가가 얼마를 벌었는지는
올해를 계획할 때 이미 알고 있는 정보인데, 그걸 버리고 있었던 셈이다.**

| 모델 | Test R2 | RMSE | MAE | fold 편차 |
|---|---:|---:|---:|---:|
{prows}

같은 부분집합({ap['패널연결_행']:,}행, 전체의 {ap['비율_pct']}%)에서 다시 잰 값이다.
**작년 ROI 한 항목을 선형 보정한 것만으로 R2 0.2548**이 나와, 설명변수 22개를 쓴 기존
모델(0.0635)의 네 배다. 패널 변수를 넣은 XGBoost는 {f(pb.get('R2', 0))}로
산림청 방식({f(g['산림청 방식']['R2'])}) 대비 **{ratio_p:.1f}배**다.

> **개선의 대부분은 모델이 아니라 자료에서 나왔다.** 작년값 선형보정만으로 0.2548이고,
> 모델이 그 위에 더한 몫은 +0.025에 불과하다. 이 점을 감추지 않는다. 여기서 읽어야 할 것은
> 알고리즘의 우수성이 아니라 **임가 수익률에는 조사표에 잡히지 않는 고유한 몫이 크고,
> 작년 실적이 그 대리 지표 노릇을 한다**는 사실이다.

과거로 배워 미래를 맞히는 조건 — 실제 사용 상황과 같다 — 에서도 앞선다.

| 분할 | 학습 | 시험 | 산림청 방식 | 기존 22변수 | 패널 변수 추가 |
|---|---:|---:|---:|---:|---:|
{yrows}
...
"""
    ...
    au = load("audit_split.json", required=False)
    audit_block = ""
    if au:
        arow, agrp = au["행단위"]["xgboost"], au["임가단위_5회평균"]["xgboost"]
        agsd = au["임가단위_5회평균"]["R2_표준편차"]
        ab0, ab1 = au["행단위"]["baseline"]["R2"], au["임가단위_5회평균"]["baseline"]["R2"]
        audit_block = f"""
### 4-5. 분할 감사 — 스스로 의심하고 확인하기

패널 자료를 행 단위로 무작위 분할하면 같은 임가가 학습과 시험 양쪽에 들어간다. 임가번호는
설명변수에 없지만 지역·업종·규모·경영비·자본의 조합이 사실상 임가를 특정하므로, 외워서
맞힐 여지가 있다. **그러면 시험 성적이 실제 일반화 능력보다 좋게 나온다.**

실제로 시험셋의 **{au['행단위_시험셋_임가중복_비율_pct']}%**가 학습셋과 임가를 공유하고 있었다.

| 분할 방식 | XGBoost R2 | 베이스라인 R2 | 개선 배수 |
|---|---:|---:|---:|
| 행 단위 (현행) | {f(arow['R2'])} | {f(ab0)} | {arow['R2'] / ab0:.2f}배 |
| 임가 단위 (5회 평균) | {f(agrp['R2'])} | {f(ab1)} | {agrp['R2'] / ab1:.2f}배 |

차이는 {f(au['R2_차이'])}로 fold 간 표준편차({f(agsd)}) 안이다.
**{au['판정']}.** 보고한 수치는 그대로 유효하다.

감사 코드는 `src/audit_split.py`에 있다. **결과가 어느 쪽으로 나오든 그대로 싣는다는
전제로 짰다.**

---
"""

    pf = load("portfolio.json", required=False)
    pfo_block = ""
    if pf:
        frows = "\n".join(
            f"| {r['품목']} | {f(r['기대수익_pct'], 1)}% | ±{f(r['연도변동_pct'], 1)}%p | "
            f"±{f(r['임가격차_pct'], 1)}%p | {f(r['효율'], 2)} | {r['표본수']:,} |"
            for r in sorted(pf["품목"], key=lambda x: -(x["효율"] or 0)))
        bp = pf["최고효율"]
        mix = " · ".join(f"{c['품목']} {c['비중_pct']}%" for c in bp["구성"])
        ratio_f = bp["임가격차_pct"] / bp["연도변동_pct"]
        pfo_block = f"""
### 4-11. 작목 조합의 위험 분산 — 그리고 예상 밖의 발견

임업 조언은 대개 "무엇이 가장 돈이 되는가"만 답한다. 그러나 임가가 실제로 겪는 문제는
**그해 시세와 작황이 흔들린다**는 것이다. 금융의 평균-분산 접근을 임산물 ROI에 적용했다.

**위험을 어떻게 쟀는가 — 이 대목이 결정적이다.** ROI가 흩어지는 원인은 두 가지다.
연도 효과(그해 시세·작황)와 임가 효과(같은 해 같은 작목 안에서도 임가마다 다름)다.
**작목을 섞어 줄일 수 있는 것은 앞의 것뿐이다.** 뒤의 것은 그 임가의 특성이라 어느 작목을
하든 따라온다. 밤과 대추를 반씩 해도 서툰 사람은 양쪽 다 서툴다.

> 초기 계산에서 임가 간 산포(σ 117~196%p)를 그대로 위험으로 놓았더니 조합 변동폭이
> **3.4%p**라는 비현실적 값이 나왔다. √n 효과의 허구다. 연도 효과(σ 10.7~39.5%p)만
> 분산 대상으로 재정의해 교정했다.

| 작목 | 기대수익 | 연도 변동 σ | 임가 격차 σ | 효율 | 표본 |
|---|---:|---:|---:|---:|---:|
{frows}

**최적 조합** — {mix} → 기대수익 {bp['기대수익_pct']}%, 연도 변동 **±{bp['연도변동_pct']}%p**,
효율 {bp['효율']}. 단일 최고효율({pf['단일_최고효율']['품목']} {pf['단일_최고효율']['효율']}) 대비
**{pf['분산효과_pct']:+.1f}%** 개선. 작목 간 상관을 일률적으로 0.3으로 보는 보수적 가정에서도
효율 {pf['보수가정']['효율']}로 결론이 유지된다.

> **그런데 더 중요한 발견이 있다.** 최적 조합의 연도 변동은 ±{bp['연도변동_pct']}%p인데
> **임가 격차는 ±{bp['임가격차_pct']}%p로 {ratio_f:.1f}배**다.
> **무엇을 심을지 고르는 것보다 어떻게 하느냐가 수익을 훨씬 크게 가른다.**
> 이것은 본 과제의 출발점 — "집단 평균이 아니라 임가별로 계산해야 한다" — 를 데이터가
> 스스로 증명한 결과다. 부수적 발견이 오히려 프로젝트 전체의 논지를 뒷받침하게 되었다.
...
- 현행 방식(지역별×업종별 단순 평균)을 예측기로 사용하면 설명력이
  **R² = {f(ma['forest_service_baseline']['test']['R2'])}** 에 그친다.
  평균값은 개별 임가 성과의 **{ma['forest_service_baseline']['test']['R2'] * 100:.1f}%밖에 설명하지 못한다.**
- 학습셋 기준 임가 간 ROI 표준편차가 **{f(dsa['roi_train_std'], 1)}%p** 로, 같은 업종·지역
  안에서도 성과 편차가 매우 크다.
...
| ⑤ 정의역 정제 | 임업 미영위 임가(경영비 0 또는 총수입 0) 제외 | A: {meta_a['dropped_no_forestry_activity']:,}건 제외 |
| ⑥ IQR 이상치 제거 | 목표변수에 1.5×IQR, 비용 항목에 3.0×IQR. Model B는 단위 분모가 달라 **품목별로** 적용 | A: {meta_a['outliers_removed']:,}건 / B: {meta_b['outliers_removed'] if meta_b else 0:,}건 |
| ⑦ 파생변수 | 구간 범주의 대표값 수치화, ha당 지표, 로그변환, 비목 구성비, 노동 배분 | A: {dsa['n_features']}개 / B: {mb['dataset']['n_features'] if mb else 0}개 |

**최종 분석 표본** — Model A **{dsa['rows']:,}행**(원시 {meta_a['raw_rows']:,}행),
Model B **{meta_b['rows_final'] if meta_b else 0:,}행**(원시 {meta_b['raw_rows'] if meta_b else 0:,}행)
...
| **코드 체계 변경** | 「경영수준별」이 2020~2022년은 5·6, 2023~2024년은 1·2 | **5→1, 6→2로 조화 ({meta_b['level_code_harmonized'] if meta_b else 0:,}행)** |
...
> Model A 제외: {', '.join(meta_a['leaky_excluded'])}
...
**{f(mb['optuna_xgboost']['test']['R2']) if mb else '—'}** 로 내려왔다.
**이것이 사전 정보만으로 얻을 수 있는 실제 예측력이다.**

### 3-5. 분석 절차

1. **데이터 분할** — Train {dsa['train']:,} / Validation {dsa['valid']:,} / Test {dsa['test']:,}
   (80:10:10, `random_state={dsa['seed']}`). Model B는 품목별 층화 추출.
2. **베이스라인 3종 구성**
...
3. **하이퍼파라미터 최적화** — Optuna TPE(multivariate) **{oxa['n_trials']}회 시행**,
   목적함수는 학습셋 **{oxa['n_folds']}-fold 교차검증 OOF RMSE**. 단일 검증셋 과적합을 막기 위해
   시행마다 {oxa['n_folds']}개 모델을 학습한다 (총 {oxa['n_trials'] * oxa['n_folds']:,}회 부스팅 학습).
...
- **GPU 가속** — `tree_method='hist'`, `device='cuda'`. Optuna 워커 스레드를
  **NVIDIA RTX A6000 {oxa.get('gpus_used', 2)}장에 라운드로빈 배정**해 병렬 탐색.
...
- 제안 모델의 R²는 **{f(oxa['test']['R2'])}** 로 현행 산림청 단순평균 방식
  ({f(ma['forest_service_baseline']['test']['R2'])}) 대비 **{impa['R2_ratio']:.1f}배**
- 예측 오차는 **RMSE {impa['RMSE_reduction_pct']:.1f}%, MAE {impa['MAE_reduction_pct']:.1f}% 감소**
- 교차검증 OOF(RMSE {f(oxa['cv_oof']['RMSE'], 2)}, R² {f(oxa['cv_oof']['R2'])})가 Test 성능과
  유사해 **과적합 없이 일반화**되었다
...
{b_block}{panel_block}{audit_block}{q_block}{weather_block}{ins_block}{prod_block}{mgmt_block}{subs_block}{pfo_block}
---
...
| **예측 정확도** | R² {f(ma['forest_service_baseline']['test']['R2'])} / MAE {f(ma['forest_service_baseline']['test']['MAE'], 1)}%p | **R² {f(oxa['test']['R2'])} / MAE {f(oxa['test']['MAE'], 1)}%p** (오차 {impa['MAE_reduction_pct']:.1f}% 감소) |
| **불확실성 안내** | 없음 (평균값이 확정값처럼 읽힘) | **예측구간 제시** (포함률 {pct(mq['roi']['coverage_80pct'] * 100, 0) if mq else '—'}) |
...
1. **경영비 최적 투입 안내** — 예측 오차가 현행 방식 대비 MAE 기준
   {impa['MAE_reduction_pct']:.1f}% 작다. 경영비 1,500만원 임가 기준으로 환산하면 임업소득 예측
   오차가 약 {(ma['forest_service_baseline']['test']['MAE'] - oxa['test']['MAE']) / 100 * 15_000_000:,.0f}원
